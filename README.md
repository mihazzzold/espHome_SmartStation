...
Если ваш репозиторий приватный, **автоматические обновления через GitHub временно не поддерживаются** из-за ограничений текущей версии ESPHome по работе с заголовками авторизации в компоненте `http_request`.

**Рекомендуемые действия:**
1. Сделайте репозиторий публичным для работы OTA.
2. Либо используйте локальный сервер обновлений (OTA-server в составе проекта).
3. Следите за обновлениями: поддержка динамических заголовков в `http_request` ожидается в будущих версиях.
